...
// Store liked posts in localStorage to persist across page views
function initializeHeartReactions() {
    addHeartStyles();
    ...
    heartContainers.forEach(container => {
        ...
        // Check if this post was previously liked from localStorage
        ...
        clickableElements.forEach(element => {
            element.addEventListener('click', (e) => {
                ...
                // Update all instances of this post's heart icon on the page
                updateAllHeartIcons(postId, isLiked, count);
...
// Add this new function to update all heart icons for a specific post
function updateAllHeartIcons(postId, isLiked, count) {
    // Find all heart containers for this post ID
    const allHeartContainers = document.querySelectorAll(`.heart-ai[data-post-id="${postId}"]`);
    
    allHeartContainers.forEach(container => {
        // Skip the container that triggered the update to avoid infinite loops
        if (container.getAttribute('data-updating') === 'true') return;
...
function showDetail(postId) {
    ...
    showDetail = function(postId) {
        ...
        setTimeout(() => {
            ...
            // Initialize heart reactions in the detail view
            initializeHeartReactions();
        }, 100);
    };
    ...
    // Ensure the heart in the detail view reflects the current state
    const detailHeartContainer = postContent.querySelector(`.heart-ai[data-post-id="${postId}"]`);
    if (detailHeartContainer) {
        detailHeartContainer.setAttribute('data-liked', isLiked);
        const heartIcon = detailHeartContainer.querySelector('.heart-icon');
        const likeCount = detailHeartContainer.querySelector('.like-count');
